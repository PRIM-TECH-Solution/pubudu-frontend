...
const CreateEvent = () => {
  ...
  const generateEventCardId = () => {
    const now = new Date();
    const eventCardId = `${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}${now.getHours().toString().padStart(2, '0')}${now.getMinutes().toString().padStart(2, '0')}${now.getSeconds().toString().padStart(2, '0')}`;
    return eventCardId;
  };
    ...
    if (validateForm()) {
      try {
        ...
        try {
          ...
          const ticketResponse = await axios.post('https://user-event.azurewebsites.net/ticketDetails/add', ticketDetailsList, {
            headers: {
