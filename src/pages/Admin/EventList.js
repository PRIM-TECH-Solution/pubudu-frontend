...
const EventList = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get('https://user-event.azurewebsites.net/eventcards/getAll');
  ...
  const deleteEvent = async (eventId) => {
    try {
      ...
      await axios.delete(`https://user-event.azurewebsites.net/eventcards/${eventId}`, {
        headers: {
  ...
  const handleEditSubmit = async (updatedEvent) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://user-event.azurewebsites.net/eventcards/${updatedEvent.eventId}`, updatedEvent, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
